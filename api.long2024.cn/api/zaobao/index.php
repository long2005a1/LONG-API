...
$font=realpath('60s/13.ttf');
$date_font=realpath('60s/2.ttf');
$image_path='60s/60.jpg';
list($width,$height)=getimagesize($image_path);
//获取高宽
$image=imagecreatefrompng($image_path);
$back=imagecolorallocate($image,00,00,00);

$box=imagettfbbox(25,0,$font,date("m")."月".date("d")."日");
imagettftext($image,25,0,600,200,$back,$font,date("Y"));
imagettftext($image,25,0,($width-$box[2]-146),200,$back,$font,date("m"));

imagettftext($image,25,0,($width-$box[2]-94),200,$back,$font,date("d"));

imagettftext($image,25,0,620,130,$back,$font,date('Y').date("m").date("d")."  ".date("d"));

//星期几
imagettftext($image,25,0,880,200,$back,$font,date("D"));
//农历
$today=new today();
$today=$today->convertSolarToLunar(date('Y'),date('m'),date('d'));
imagettftext($image,25,0,80,380,$back,$font,"农历");
imagettftext($image,25,0,80,420,$back,$font,$today[1].$today[2]);
//新闻
clearstatcache();//清除缓存
$new_path="新闻.txt";
if(file_exists($new_path) && date("Y-m-d") == date("Y-m-d", filemtime($new_path)))
{
$news=file_get_contents($new_path);
}else{
$news='';
...
for($i=0;$i!=9;$i++)
{
$new=($i+1).".".str_replace(array("<br>","&nbsp;","&quot;"),array(""," ",'"'),$data['cards'][0]['content'][$i]['query']) . '  ' . $data['cards'][0]['content'][$i]['desc'];
$news.=chunkSplit($new,25,"\n")."\n";
}
file_put_contents($new_path,$news);
}
imagettftext($image,25,0,80,500,$back,$font,$news);
//微语
$yiyan=Curl::get('https://api.long2024.cn/api/mryy')->body();
imagettftext($image,25,0,100,1811,$back,$font,"{$yiyan}");
//版权
imagettftext($image,25,0,620,1755,$back,$font,"每日早报 BY小易API");
//显示图片
imagejpeg($image);
//销毁图片
imagedestroy($image);
 ...
 */
	function getCapitalNum($num,$isMonth) {
		...
		$res='';
		if($isMonth) $res = $monthHash[$num]; else {
			if($num<=10) $res = '初'.$dateHash[$num]; else if($num>10&&$num<20) $res = '十'.$dateHash[$num-10]; else if($num==20) $res = "二十"; else if($num>20&&$num<30) $res = "廿".$dateHash[$num-20]; else if($num==30) $res = "三十";
		}
		return $res;
	}
	/*
